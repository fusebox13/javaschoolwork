...
public class DiagonalLayout implements LayoutManager {
    ...
    private void setSizes(Container parent) {
        ...
        System.out.println("preferredWidth = "+preferredWidth + " preferredHeight =" +
                    preferredHeight);
        System.out.println("minWidth = "+minWidth + " minHeight =" +
                    minHeight);
     ...
     * minimumLayoutSize will be called -- 
     */
    public void layoutContainer(Container parent) 
    {
        ...
        for (int i = 0 ; i < nComps ; i++) 
        {
            ...
            if (c.isVisible()) {
                ...
                // If x is too large, 
                if ((!oneColumn) && (x + d.width) > (parent.getSize().width - insets.right)) 
                {
                    // reduce x to a reasonable number.
                    x = parent.getSize().width - insets.right - d.width;
                }

                // If y is too large, 
                if ((y + d.height) > (parent.getSize().height - insets.bottom)) 
                {
                    // do nothing.
